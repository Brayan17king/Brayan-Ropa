...
namespace API.Controllers
{
    public class VentaController : BaseController
    {
        ...
        public async Task<ActionResult<VentaDto>> Post(VentaDto resultDto)
        {
            ...
            if (resultDto.FechaVenta == DateOnly.MinValue)
            {
                result.FechaVenta = DateOnly.FromDateTime(DateTime.Now);
                resultDto.FechaVenta = DateOnly.FromDateTime(DateTime.Now);
            }
        ...
        public async Task<ActionResult<VentaDto>> Put(int id, [FromBody] VentaDto resultDto)
        {
            ...
            if (resultDto.FechaVenta == DateOnly.MinValue)
            {
                exists.FechaVenta = DateOnly.FromDateTime(DateTime.Now);
            }
            await _unitOfWork.SaveAsync();
