...
namespace API.Controllers
{
    public class OrdenController : BaseController
    {
        ...
        public async Task<ActionResult<OrdenDto>> Post(OrdenDto resultDto)
        {
            ...
            if (resultDto.FechaOrden == DateOnly.MinValue)
            {
                result.FechaOrden = DateOnly.FromDateTime(DateTime.Now);
                resultDto.FechaOrden = DateOnly.FromDateTime(DateTime.Now);
            }
        ...
        public async Task<ActionResult<OrdenDto>> Put(int id, [FromBody] OrdenDto resultDto)
        {
            ...
            if (resultDto.FechaOrden == DateOnly.MinValue)
            {
                exists.FechaOrden = DateOnly.FromDateTime(DateTime.Now);
            }
            await _unitOfWork.SaveAsync();
